...
pub fn main() !void {
    ...
    const User = struct {
        ...
        test_value: []const u8,
        test_num: ?i32, //The field can be made optional
        test_bool: bool,
    };

    {
        ...
        var insert = db.insert(User, .{ .test_value = "foo", .test_bool = false });
...

    {
        ...
        try select.where(.{ .test_value = "foo", .test_bool = false });
...

    {
        ...
        var insert = db.insert(PrimaryKey_User, .{ .test_value = "foo" });
        ...
        try select.where(.{ .test_value = "foo" });
...

    {
        ...
        var delete = db.delete(User, .{ .test_value = "hoo", .test_bool = true });
